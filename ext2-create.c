...
void write_superblock(int fd) {
	...
	superblock.s_checkinterval     = EXT2_ERRORS_CONTINUE; /* Force checks by making them every 1 second */
...
void write_block_bitmap(int fd) {
	...
	for (size_t i = 0; i < BLOCK_SIZE; i ++){
		if (i ==2) buffer[i] = 0b01111111;
		else if (i>2 && i < 127) buffer[i] = 0;
		else if (i == 127) buffer[i] = 0b10000000;
		else buffer[i] = 0xFF;
	}
...
void write_inode_bitmap(int fd) {
	...
	uint8_t buffer[BLOCK_SIZE]={0x00};
	buffer[0]= 0xFF;
	buffer[1]=0b00011111;

	for (size_t i = NUM_INODES/8; i<BLOCK_SIZE; i++){
		buffer[i] = 0xFF;
	}
